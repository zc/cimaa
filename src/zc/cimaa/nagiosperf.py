r"""Parser for nagios performance data
    ...
    ('DISK OK - free space: / 3326 MB (56%);
       ...
       (27%);\n/var/log 819 MB (84%); ',
     [{'name': '/', 'units': 'MB', 'value': '2643'},
      {'name': '/boot', 'units': 'MB', 'value': '68'},
      {'name': '/home', 'units': 'MB', 'value': '69357'},
      {'name': '/var/log', 'units': 'MB', 'value': '818'}])

    >>> ppp("PING ok - Packet loss = 0%, RTA = 0.80 ms "
    ...              "| percent_packet_loss=0, rta=0.80")
    ('PING ok - Packet loss = 0%, RTA = 0.80 ms \n',
     [{'name': 'percent_packet_loss', 'units': ',', 'value': '0'},
      {'name': 'rta', 'units': '', 'value': '0.80'}])

    >>> ppp("| 'ha ha ha'=3has")
    ('\n', [{'name': "'ha ha ha'", 'units': 'has', 'value': '3'}])
...
def parse_output(text):
    ...
    return (
        first[0] + '\n' + rest[0],
        [dict(name=m[0], value=m[1], units=m[3]) for m in perf_parse(perf)],
        )
